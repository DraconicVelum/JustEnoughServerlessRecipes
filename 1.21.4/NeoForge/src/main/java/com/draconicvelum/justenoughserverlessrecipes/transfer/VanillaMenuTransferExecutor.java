...
final class VanillaMenuTransferExecutor {
    ...
    public static boolean execute(
        ...
        for (Slot craftingSlot : craftingSlots) {
            if (!craftingSlot.getItem().isEmpty()) {
                click(container.containerId, craftingSlot.index, 0, ClickType.QUICK_MOVE, player);
            }
        }
        ...
        for (PlannedMove plannedMove : plannedMoves) {
            click(container.containerId, plannedMove.sourceSlot(), 0, ClickType.PICKUP, player);
            click(container.containerId, plannedMove.targetSlot(), 1, ClickType.PICKUP, player);
            if (!container.getCarried().isEmpty()) {
                click(container.containerId, plannedMove.sourceSlot(), 0, ClickType.PICKUP, player);
            }
        }
    ...
    private static void click(int containerId, int slotNum, int buttonNum, ClickType clickType, Player player) {
        try {
            Object minecraft = getMinecraftInstance();
            Object gameMode = getGameMode(minecraft);
            if (gameMode == null) {
                throw new IllegalStateException("Minecraft gameMode is null");
            }

            Method handleInventoryMouseClick = getInventoryClickMethod(gameMode);
            handleInventoryMouseClick.invoke(gameMode, containerId, slotNum, buttonNum, clickType, player);
    ...
    private static Object getMinecraftInstance() throws ReflectiveOperationException {
        ...
        for (Method method : minecraftClass.getDeclaredMethods()) {
            if (Modifier.isStatic(method.getModifiers())
                    ...
                    && minecraftClass.isAssignableFrom(method.getReturnType())) {
                method.setAccessible(true);
                return method.invoke(null);
            }
        }
        throw new NoSuchMethodException("Minecraft singleton accessor");
    }

    private static Class<?> getMinecraftClass() throws ClassNotFoundException {
        try {
    ...
    private static Object getGameMode(Object minecraft) throws ReflectiveOperationException {
        for (Field field : minecraft.getClass().getDeclaredFields()) {
            field.setAccessible(true);
            Object value = field.get(minecraft);
            if (value != null && hasInventoryClickMethod(value)) {
                return value;
            }
        }
        return null;
    ...
    private static boolean hasInventoryClickMethod(Object value) {
        try {
            getInventoryClickMethod(value);
            return true;
    ...
    private static Method getInventoryClickMethod(Object gameMode) throws NoSuchMethodException {
        for (Method method : gameMode.getClass().getDeclaredMethods()) {
            Class<?>[] parameterTypes = method.getParameterTypes();
            if (parameterTypes.length == 5
                    && parameterTypes[0] == int.class
                    && parameterTypes[1] == int.class
                    && parameterTypes[2] == int.class
                    && ClickType.class.isAssignableFrom(parameterTypes[3])
                    && Player.class.isAssignableFrom(parameterTypes[4])) {
                method.setAccessible(true);
                return method;
            }
        }
        throw new NoSuchMethodException("inventory click handler");
    }
